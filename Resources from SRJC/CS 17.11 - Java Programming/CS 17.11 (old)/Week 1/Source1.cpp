...
void count(ifstream& inFile, letterType letterList[], int& totalLetter);

//Function to print the letters, the occurences, and the percentages
void printResult(ofstream& outFile, letterType letterList[], int totalLetter);
...
void printResult(ofstream& outFile, letterType letterList[], int totalLetter)
{
	...
	for (int index = 0; index < 26; index++)
	{
		if (0 <= index && index <= 25)
		{
			...
			outFile << setw(15) << 100 * (letterList[index].letterCount / static_cast<double>(totalLetter)) << "%" << endl;
		}
	} //end for
} //end function 
